...
- `"external"` (`ACP_BACKEND_EXTERNAL`): an **operator-declared** harness — the one entry whose subprocess this module does not know how to find. There is no binary name to resolve: the argv arrives as `AcpClient(acp_command=[...])`, built by `config.loader.resolve_acp_interface` from the `acp_interfaces.<name>` section and selected per crew by `agents.<crew>.acp_interface` (see configuration.md and the operator guide `docs/guides/acp-interfaces.md`). `command` must be an **absolute** path — `_parse_acp_interfaces` refuses a relative one at load — and the resolver stores its `realpath`, because a relative command would resolve against the gateway's cwd when validated and against the session work dir when spawned, and that work dir is workspace-controlled. `_spawn` re-checks launchability through `kiro_prerequisite._acp_executable_is_runnable` (the same predicate the Kiro prerequisite applies to its own binary — not a second spelling of the rule) and raises `AcpError` naming the config key rather than dying at handshake. Deliberately absent on this branch, unlike the Kiro one: no binary search, and no `ensure_agent_materialized` — `~/.kiro/agents/` is kiro-cli's agent format, and writing it for a foreign harness would imply a contract neither side agreed to; the harness receives the agent name in its argv via the `{agent}` placeholder and decides for itself what it means. It is in no capability set and gets no `cli.json` overlay (that file is kiro-cli's own), keeps Kiro Crew's OS sandbox because it is **not** in `ACP_BACKENDS_INTERNAL_SANDBOX`, and has Kiro's API key stripped from its environment by `_resolve_spawn_env(kiro_api_key=self._is_kiro)`. Its Group B decisions are recorded in harness-parity.md under "Registered harnesses". Tool gating still runs on `session/request_permission`, so a harness that executes tools without asking is not gated — declaring an interface is an operator's deliberate trust grant.
